...
class FieldExtractor:
        ...
        try:
            ...
            for doc in data.get("documents", []):
                ...
                for field in extracted_doc.get("extracted_fields", []):
                    result["extraction_summary"]["extracted_fields"].add(field["field_name"])
            ...
            return {"error": f"Ошибка при чтении JSON файла: {str(e)}"}
    ...
    def extract_from_xml(self, xml_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Ошибка при чтении XML файла: {str(e)}"}
    ...
    def _extract_document_fields(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        basic_fields = ["document_id", "document_type", "document_number", "document_date", 
                       "is_posted", "total_amount", "description", "counterparty"]
        ...
        for field_name, value in all_fields.items():
            if field_name.startswith("_FLD"):
                ...
                result["extracted_fields"].append({
                    ...
                    "field_description": self.field_mapping.get(field_name, "Неизвестное поле")
                })
        ...
        result["quality_score"] = (extracted_count / total_fields * 100) if total_fields > 0 else 0
    ...
    def _extract_xml_document(self, doc_elem: ET.Element) -> Dict[str, Any]:
        ...
        # Извлекаем основные поля
        basic_fields = ["ID", "Type", "Number", "Date", "Posted", "TotalAmount", "Description", "Counterparty"]
    ...
    def _interpret_field(self, field_name: str, value: Any) -> str:
        ...
        if field_name in ["_POSTED", "_MARKED", "_FLD4225", "_FLD4226", "_FLD4227", 
                         "_FLD4236", "_FLD4237", "_FLD4249", "_FLD8205", "_FLD10654", 
                         "_FLD13609", "_FLD14340"]:
            return "Булево значение" if isinstance(value, bool) else f"Логическое поле: {value}"
        
        elif field_name in ["_FLD4238", "_FLD4239", "_FLD9885"]:
            return f"Финансовая сумма: {value}" if isinstance(value, (int, float)) else f"Финансовое поле: {value}"
        
        elif field_name in ["_FLD4240", "_FLD8015", "_FLD10651"]:
            return f"Числовое значение: {value}" if isinstance(value, (int, float)) else f"Числовое поле: {value}"
        
        elif field_name in ["_FLD4252", "_FLD8070", "_FLD12955", "_FLD12950"]:
            return f"Текстовое поле: {value}" if isinstance(value, str) else f"Текстовое поле: {value}"
        
        elif field_name in ["_NUMBER", "_VERSION"]:
...
def main() -> None:
    ...
    json_result = extractor.extract_from_json("[prostocvet-1c]/raw/clean_working_documents.json")
    
    if "error" in json_result:
        print(f"Ошибка: {json_result['error']}")
    else:
        print(f"Извлечено документов: {json_result['extraction_summary']['total_documents']}")
        print(f"Успешных извлечений: {json_result['extraction_summary']['successful_extractions']}")
        print(f"Неудачных извлечений: {json_result['extraction_summary']['failed_extractions']}")
        print(f"Уникальных полей: {len(json_result['extraction_summary']['extracted_fields'])}")
        print(f"Отсутствующих полей: {len(json_result['extraction_summary']['missing_fields'])}")
    ...
    xml_result = extractor.extract_from_xml("[prostocvet-1c]/raw/clean_working_documents.xml")
    
    if "error" in xml_result:
        print(f"Ошибка: {xml_result['error']}")
    else:
        print(f"Извлечено документов: {xml_result['extraction_summary']['total_documents']}")
        print(f"Успешных извлечений: {xml_result['extraction_summary']['successful_extractions']}")
        print(f"Неудачных извлечений: {xml_result['extraction_summary']['failed_extractions']}")
        print(f"Уникальных полей: {len(xml_result['extraction_summary']['extracted_fields'])}")
        print(f"Отсутствующих полей: {len(xml_result['extraction_summary']['missing_fields'])}")
