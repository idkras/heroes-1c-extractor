...
class TestCaseRunner:
    ...
    def generate_report(self, results):
        ...
        # Сохранение отчета
        report_file = self.reports_dir / f"testcase_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    if not runner.results_dir.exists():
        print("❌ Папка с результатами не найдена. Запустите извлечение данных сначала.")
        sys.exit(1)
