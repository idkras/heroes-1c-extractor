...
class TestNotebookQA:
    ...
    def notebook_path(self):
        ...
        return (
            Path(__file__).parent.parent.parent / "notebooks" / "parquet_analysis.ipynb"
        )
    ...
    def data_paths(self):
        """Пути к данным 1С"""
        return {
            "documents_parquet": Path(__file__).parent.parent.parent
            / "data"
            / "results"
            / "parquet"
            / "documents.parquet",
            "analysis_duckdb": Path(__file__).parent.parent.parent
            / "data"
            / "results"
            / "duckdb"
            / "analysis.duckdb",
            "test_flowers_parquet": Path(__file__).parent.parent.parent
            / "data"
            / "results"
            / "test_flowers.parquet",
            "test_flowers_duckdb": Path(__file__).parent.parent.parent
            / "data"
            / "results"
            / "test_flowers.duckdb",
        }
    ...
    def test_data_consistency(self, data_paths):
        ...
        if (
            data_paths["documents_parquet"].exists()
            and data_paths["analysis_duckdb"].exists()
        ):
            ...
            assert len(df_parquet) == len(
                df_duckdb,
            ), "Разное количество записей в Parquet и DuckDB"
            assert list(df_parquet.columns) == list(
                df_duckdb.columns,
            ), "Разные колонки в Parquet и DuckDB"
