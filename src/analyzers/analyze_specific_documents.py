...
def analyze_specific_documents():
    """
    Детальный анализ конкретных документов по критериям из 1c.todo.md
    """
    print("🔍 Детальный анализ документов по критериям")
    print("=" * 60)
    ...
    try:
        with open('raw/1Cv8.1CD', 'rb') as f:
            ...
            for table_name, record_count in document_tables[:10]:
                ...
                # Анализируем первые 20 записей для классификации
                sample_records = []
                ...
                if sample_records:
                    ...
                    blob_fields = []
                    for field in fields:
                        if field.startswith('_FLD') and any(record.get(field) for record in sample_records if hasattr(record.get(field), 'value')):
                    ...
                    if document_classification:
                        ...
                        for doc_type, classification in document_classification.items():
                            ...
                            for reason in classification['reasons'][:3]:  # Показываем первые 3 причины
                                print(f"        - {reason}")
                                ...
                                try:
                                    ...
                                    if hasattr(value, 'value'):
                                        ...
                                        if content and len(str(content)) > 0:
                                            print(f"        Запись {i+1}: {str(content)[:100]}...")
            ...
            # Сохраняем результаты
            output_file = 'specific_documents_analysis.json'
