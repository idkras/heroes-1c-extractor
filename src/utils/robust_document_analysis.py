    ...
    try:
        if hasattr(value, "value"):
            ...
            try:
                ...
                logger.debug(f"Ошибка при обработке содержимого: {e}")
    except Exception as e:
        ...
        with open("raw/1Cv8.1CD", "rb") as f:
            ...
            search_criteria = {
                "поступления": {
                    "conditions": [
                        "Проведён = Истина",
                        "СкладОрдер = Центральный склад",
                        "НомерМашины заполнено",
                    ],
                    "keywords": [
                        "поступление",
                        "приход",
                ...
                "перемещения": {
                    "conditions": [
                        "Проведён = Истина",
                        "СкладОтправитель = Центральный склад",
                        "СкладПолучатель = Интернет-магазин",
                    ],
                    "keywords": [
                        "перемещение",
                        "отправитель",
            ...
            print("🔍 Анализируем первые 8 больших таблиц...")
                ...
                if sample_records:
                    ...
                    for doc_type, criteria in search_criteria.items():
                        ...
                        for keyword in criteria["keywords"]:
                            matching_fields = [
                                f for f in fields if keyword.lower() in f.lower()
                            ]
                            if matching_fields:
                    ...
                    for field in fields[:8]:  # Анализируем первые 8 полей
                        ...
                        if values:
                            field_values[field] = list(set(values))[
                                :5
                            ]  # Уникальные значения
                    ...
                    if blob_fields:
                        ...
                        for blob_field in blob_fields[
                            :2
                        ]:  # Анализируем первые 2 BLOB поля
                    ...
                    results["documents_found"][table_name] = {
                        ...
                        "sample_records": sample_records[
                            :2
                        ],  # Сохраняем первые 2 записи
                    }

                    results["metadata"]["total_tables_analyzed"] = (
            ...
            with open(output_file, "w", encoding="utf-8") as f:  # type: ignore
                json.dump(results, f, ensure_ascii=False, indent=2, default=str)  # type: ignore
            ...
            for table_name, data in results["documents_found"].items():
                if data.get("document_classification"):
                    print(f"  📄 {table_name}:")
                    for doc_type, classification in data[
