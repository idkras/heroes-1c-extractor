...
def search_all_document_types():
    ...
    try:
        with open('raw/1Cv8.1CD', 'rb') as f:
                ...
                try:
                    ...
                    if len(table) > 0:
                        ...
                        if not first_record.is_empty:
                            ...
                            for field_name, field_value in list(record_data.items())[:10]:
                            ...
                            if blob_fields:
                                print(f"    🔍 BLOB поля ({len(blob_fields)}): {blob_fields[:5]}")
                                    ...
                                    try:
                                        ...
                                        if hasattr(blob_value, 'value'):
                                            ...
                                            if content and len(str(content)) > 0:
                                                print(f"        📋 {blob_fields[0]}: {str(content)[:100]}...")
                                    except Exception as e:
                                        print(f"        ⚠️ Ошибка чтения BLOB: {e}")
                            ...
                            table_info = {
                                ...
                                'sample_data': {k: v for k, v in list(record_data.items())[:5] if not str(v).startswith('<onec_dtools.database_reader.Blob')}
                            }
                ...
                try:
                    ...
                    if len(table) > 0:
                        ...
                        if not first_record.is_empty:
                            ...
                            # Показываем основные поля
                            print(f"    📄 Основные поля:")
                            for field_name, field_value in list(record_data.items())[:8]:
                                if not str(field_value).startswith('<onec_dtools.database_reader.Blob'):
                                    print(f"        📋 {field_name}: {field_value}")
                            ...
                            if blob_fields:
                                print(f"    🔍 BLOB поля ({len(blob_fields)}): {blob_fields[:3]}")
            ...
            for keyword, description in keywords.items():
                ...
                for table_name in document_tables.keys():
                    if keyword.lower() in table_name.lower():
                        matching_tables.append((table_name, document_tables[table_name]))
            ...
            with open('all_document_types_analysis.json', 'w', encoding='utf-8') as f:
            ...
            # Итоговая статистика
            print(f"\n📊 ИТОГОВАЯ СТАТИСТИКА:")
            print(f"    📋 Документы: {len(results['document_types'])} типов")
            print(f"    📋 Справочники: {len(results['references'])} типов")
            print(f"    📋 Регистры накопления: {len(results['accumulation_registers'])} типов")
            print(f"    🔍 Ключевые слова найдены: {sum(1 for v in found_keywords.values() if v)} из {len(keywords)}")
