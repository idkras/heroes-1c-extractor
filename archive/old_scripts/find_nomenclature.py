...
def find_nomenclature():
    """
    Поиск таблиц со справочниками номенклатуры
    """
    print("🔍 Поиск таблиц со справочниками номенклатуры")
    print("=" * 60)
    ...
    try:
        with open('raw/1Cv8.1CD', 'rb') as f:
            ...
            # Ищем таблицы со справочниками номенклатуры
            nomenclature_tables = []
            
            for table_name in db.tables.keys():
                table_lower = table_name.lower()
                
                # Проверяем ключевые слова номенклатуры
                nomenclature_keywords = [
                    'reference', 'справочник', 'номенклатур', 'товар',
                    'наименование', 'описание', 'код', 'название'
                ]
                
                has_keyword = any(keyword in table_lower for keyword in nomenclature_keywords)
            ...
            print(f"\n✅ Найдено {len(nomenclature_tables)} таблиц со справочниками номенклатуры:")
            ...
            # Если не нашли номенклатуру, ищем в больших таблицах
            if not nomenclature_tables:
