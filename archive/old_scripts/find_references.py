...
def find_references():
    """
    Поиск справочников и номенклатуры
    """
    print("🔍 Поиск справочников и номенклатуры")
    print("=" * 60)
        ...
        with open('raw/1Cv8.1CD', 'rb') as f:
            ...
            for table_name in db.tables.keys():
                ...
                reference_keywords = [
                    ...
                    'контрагент', 'склад', 'организац', 'сотрудник'
                ]
                ...
                if has_keyword and len(db.tables[table_name]) > 0:
                    ...
                    if non_empty_count > 0:
                        reference_tables.append((table_name, len(table), non_empty_count))
            
            print(f"\n✅ Найдено {len(reference_tables)} таблиц со справочниками:")
            
            for i, (table_name, total_count, non_empty_count) in enumerate(reference_tables[:10]):
                print(f"\n{i+1}. 📊 {table_name}")
            ...
            # Если не нашли справочники, ищем в больших таблицах
