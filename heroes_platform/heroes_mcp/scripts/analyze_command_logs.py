...
class CommandLogAnalyzer:
    ...
    def analyze_recent_executions(self, days: int = 7) -> Dict[str, Any]:
        ...
        for i in range(days):
            ...
            log_file = self.log_dir / f"command_execution_{date.strftime('%Y%m%d')}.jsonl"
        ...
        if isinstance(total_executions, (int, float)) and total_executions > 0:
            ...
            if isinstance(successful_executions, (int, float)) and isinstance(failed_executions, (int, float)):
    ...
    def _process_log_entry(self, entry: Dict[str, Any], results: Dict[str, Any]) -> None:
        ...
        if "artifact_path" in data:
            ...
            if artifact_path:
                results["command_frequency"][artifact_path] = results["command_frequency"].get(artifact_path, 0) + 1
        ...
        if status == "FAILED" and "error" in data:
            error = data["error"]
            results["error_patterns"][error] = results["error_patterns"].get(error, 0) + 1
...
"""
        ...
        # Топ-5 команд
        sorted_commands = sorted(analysis["command_frequency"].items(), key=lambda x: x[1], reverse=True)
        ...
        if analysis["execution_times"]:
            avg_time = sum(analysis["execution_times"]) / len(analysis["execution_times"])
...
def main():
    ...
    # Сохраняем отчет
    report_file = Path("logs/command_execution") / f"analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
