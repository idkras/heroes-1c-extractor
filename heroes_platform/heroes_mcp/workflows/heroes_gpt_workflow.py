...
class HeroesGPTWorkflow:
    ...
    async def _execute_legacy_workflow(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            jtbd_scenarios = []
            if "crosslisting" in text_content.lower() or "cross-listing" in text_content.lower():
            ...
            recommendations = []
            if len(offers) < 5:
                recommendations.append("Add more compelling offers and value propositions")
            if not any("price" in offer["offer_text"].lower() for offer in offers):
                recommendations.append("Add pricing information")
            if not ("testimonial" in text_content.lower() or "review" in text_content.lower()):
            ...
            result = {
                ...
                "stage_outputs": {
                    "deep_segment_research": {
                        ...
                        "target_segments": ["resellers", "vintage shops", "multi-marketplace sellers"]
                    },
                ...
                "quality_score": min(95, 60 + len(offers) * 3 + len(jtbd_scenarios) * 10)
            }
    ...
    async def _convert_to_legacy_format(self, result: Dict[str, Any], landing_url: str) -> HeroesGPTReport:
        """
        ...
        landing_analysis = LandingAnalysis(
            url=landing_url,
            business_type=result.get("stage_outputs", {}).get("deep_segment_research", {}).get("business_type", "unknown"),
            main_value_prop=result.get("stage_outputs", {}).get("deep_segment_research", {}).get("main_value_prop", ""),
            target_segments=result.get("stage_outputs", {}).get("deep_segment_research", {}).get("target_segments", []),
            analysis_time=0.0,  # Will be calculated
            content_length=0  # Will be calculated
        )
        
        # Create offers table from real data
        offers_data = result.get("stage_outputs", {}).get("unified_table_methodology", {}).get("offers_analysis", [])
        offers_table = []
        for offer_data in offers_data:
            offers_table.append(OfferAnalysis(
        ...
        jtbd_data = result.get("stage_outputs", {}).get("activating_knowledge", {}).get("jtbd_scenarios", [])
        jtbd_scenarios = []
        for jtbd_item in jtbd_data:
            jtbd_scenarios.append(JTBDScenario(
        ...
        report = HeroesGPTReport(
            ...
            jtbd_scenarios=jtbd_scenarios,
            segments=result.get("stage_outputs", {}).get("deep_segment_research", {}).get("segments", {}),
            rating=min(5, max(1, result.get("quality_score", 0) // 20)),  # Convert 0-100 to 1-5
            recommendations=result.get("final_report", {}).get("recommendations", []),
            reflections=reflections,
            narrative_coherence_score=min(10, max(1, result.get("quality_score", 0) // 10)),  # Convert 0-100 to 1-10
...
__all__ = ["HeroesGPTWorkflow", "HeroesGPTReport", "OfferAnalysis", "JTBDScenario", "LandingAnalysis", "ReflectionCheckpoint"]
