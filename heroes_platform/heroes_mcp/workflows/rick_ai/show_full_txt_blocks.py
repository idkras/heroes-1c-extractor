...
def show_full_txt_blocks():
    ...
    for i, client_id in enumerate(random_sessions, 1):
        ...
        for j, event in enumerate(session_events):
            ...
            print(
                f"event_param_date_hour_minute: {event.get('event_param_date_hour_minute', '')}"
            )
            print()
            ...
            print(
                f"event_param_last_search_engine: {event.get('event_param_last_search_engine', '')}"
            )
            print(
                f"event_param_last_search_engine_root: {event.get('event_param_last_search_engine_root', '')}"
            )
            print(
                f"event_param_last_adv_engine: {event.get('event_param_last_adv_engine', '')}"
            )
            print(
                f"event_param_last_traffic_source: {event.get('event_param_last_traffic_source', '')}"
            )
            print(
                f"event_param_last_social_network: {event.get('event_param_last_social_network', '')}"
            )
            print(
                f"event_param_last_social_network_profile: {event.get('event_param_last_social_network_profile', '')}"
            )
            print()
            ...
            print()
            print(
                f"event_param_rick_user_agent: {event.get('event_param_rick_user_agent', '')}"
            )
            ...
            print(
                f"event_param_rick_ad_channel_identifiers: {event.get('event_param_rick_ad_channel_identifiers', '')}"
            )
            print(
                f"event_param_rick_additional_campaign_data: {event.get('event_param_rick_additional_campaign_data', '')}"
            )
            print(
                f"event_param_rick_ad_identifiers: {event.get('event_param_rick_ad_identifiers', '')}"
            )
            print(
                f"event_param_rick_campaign_attribution: {event.get('event_param_rick_campaign_attribution', '')}"
            )
            print(
                f"event_param_rick_fb_client_id: {event.get('event_param_rick_fb_client_id', '')}"
            )
            print("```")
            print()
            ...
            if source_medium != expected_source_medium:
                print(
                    f"🚨 ПРОТИВОРЕЧИЕ: '{source_medium}' vs '{expected_source_medium}'"
                )
            else:
                print("✅ Совпадает")
