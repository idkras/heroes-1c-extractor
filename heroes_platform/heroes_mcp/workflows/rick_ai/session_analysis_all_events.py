...
from heroes_platform.heroes_mcp.workflows.rick_ai.analysis_manager import SourceMediumRules
...
def analyze_all_events_in_session():
    ...
    print(f"📊 СЕССИОННЫЙ АНАЛИЗ ВСЕХ СОБЫТИЙ ПО CLIENTID")
    ...
    for i, client_id in enumerate(random_sessions, 1):
        ...
        for j, event in enumerate(session_events):
            ...
            print(f"\n🎯 АНАЛИЗ 71 ПОЛЯ:")
            print(f"   Ожидаемое sourceMedium: {analysis_result.get('expected_source_medium', '')}")
            print(f"   Ожидаемое rawSourceMedium: {analysis_result.get('expected_raw_source_medium', '')}")
            
            # Проверяем противоречия
            contradictions = analysis_result.get("contradictions", [])
            if contradictions:
                print(f"   🚨 ПРОТИВОРЕЧИЯ:")
        ...
        for j in range(1, len(session_events)):
            ...
            if current_source_medium == previous_source_medium:
                print(f"   Событие {j+1}: sourceMedium наследуется от события {j} = '{current_source_medium}'")
            else:
                print(f"   Событие {j+1}: sourceMedium изменилось с '{previous_source_medium}' на '{current_source_medium}'")
