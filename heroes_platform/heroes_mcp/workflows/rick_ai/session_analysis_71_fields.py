...
def analyze_sessions_by_client_id():
    ...
    print("📊 Анализ сессий по clientID - группировка событий")
    ...
    for i, client_id in enumerate(random_sessions, 1):
        ...
        if custom_group and custom_group != "google.ru":
            print(
                f"   custom_group_campaign_grouping: '{custom_group}' - указывает на источник"
            )
        ...
        if len(session_events) > 1:
            ...
            for k, prev_event in enumerate(session_events[:-1]):
                ...
                print(
                    f"   Событие {k + 1} ({prev_time}): sourceMedium='{prev_source_medium}'"
                )
        ...
        rick_ad_identifiers = last_event.get(
            "event_param_rick_ad_channel_identifiers", ""
        )
        if (
            rick_ad_identifiers
        ...
        print(
            f"   sourceMedium: '{source_medium}' vs '{expected_source_medium}' = {'✅' if source_medium_match else '❌'}"
        )
        print(
            f"   raw_source_medium: '{raw_source_medium}' vs '{expected_raw_source_medium}' = {'✅' if raw_source_medium_match else '❌'}"
        )
        ...
        if not source_medium_match or not raw_source_medium_match:
            ...
            if not source_medium_match:
                print(
                    f"      sourceMedium: Rick.ai записал '{source_medium}', должно быть '{expected_source_medium}'"
                )
            if not raw_source_medium_match:
                print(
                    f"      raw_source_medium: Rick.ai записал '{raw_source_medium}', должно быть '{expected_raw_source_medium}'"
                )
        else:
