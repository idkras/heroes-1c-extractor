...
class HeroesMCPIntegration:
    ...
    async def handle_heroes_generate_report(
        self, analysis_data: dict[str, Any], report_format: str = "markdown"
    ) -> dict[str, Any]:
    ...
    async def handle_heroes_get_recommendations(
        self, analysis_data: dict[str, Any], recommendation_type: str = "all"
    ) -> dict[str, Any]:
    ...
    async def _format_analysis_result_for_mcp(
        self, analysis_result: dict[str, Any]
    ) -> dict[str, Any]:
        """Форматирует результат анализа для MCP"""

        if not analysis_result.get("success", False):
            return await self._format_error_for_mcp(
            ...
            "heroes_validate_analysis": {
                ...
                "parameters": {
                    ...
                    "validation_standard": {
                        "type": "string",
                        "required": False,
                        "default": "Ilya Krasinsky Review Standard v1.2",
                        "description": "Стандарт валидации",
                    },
                },
    ...
    async def list_available_commands(self) -> dict[str, Any]:
        ...
        return {
            ...
            "commands_info": {
                cmd: await self.get_mcp_command_info(cmd) for cmd in self.mcp_commands
            },
    ...
    try:
        ...
        if command == "heroes_analyze_landing":
            ...
            return await integration.handle_heroes_validate_analysis(
                parameters.get("analysis_data", {}),
                parameters.get(
                    "validation_standard", "Ilya Krasinsky Review Standard v1.2"
                ),
            )
...
def main():
    ...
    async def test_mcp_integration():
        ...
        analysis_result = await execute_heroes_mcp_command(
            "heroes_analyze_landing",
            {
        ...
        if analysis_result["success"]:
            recommendations_result = await execute_heroes_mcp_command(
                "heroes_get_recommendations",
                {
