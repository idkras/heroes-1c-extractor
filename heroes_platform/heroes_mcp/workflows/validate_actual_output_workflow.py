...
from .validate_actual_output.workflow import (
    ValidateActualOutputWorkflow as CoreWorkflow,
)
    ...
    async def execute(self, input_data) -> dict:
        ...
        try:
            ...
            result["execution_metadata"] = {
                ...
                else getattr(input_data, "artifact_type", "unknown"),
                "execution_time": execution_time,
                "timestamp": datetime.now().isoformat(),
                "arguments": {
                    "artifact_path": input_data.get("artifact_path", "")
                    if isinstance(input_data, dict)
                    else getattr(input_data, "artifact_path", ""),
                    "artifact_type": input_data.get("artifact_type", "")
                    if isinstance(input_data, dict)
                    else getattr(input_data, "artifact_type", ""),
                    "expected_features": input_data.get("expected_features", "")
                    if isinstance(input_data, dict)
                    else getattr(input_data, "expected_features", ""),
                    "test_cases": input_data.get("test_cases", "")
                    if isinstance(input_data, dict)
                    else getattr(input_data, "test_cases", ""),
                    "take_screenshot": input_data.get("take_screenshot", True)
                    if isinstance(input_data, dict)
                    else getattr(input_data, "take_screenshot", True),
                    "log_execution": input_data.get("log_execution", True)
                    if isinstance(input_data, dict)
                    else getattr(input_data, "log_execution", True),
                    "url": input_data.get("url", "")
                    if isinstance(input_data, dict)
                    else getattr(input_data, "url", ""),
                },
            }

            logger.info(
                f"Validation workflow completed: {execution_id} in {execution_time:.3f}s"
            )
            return result
