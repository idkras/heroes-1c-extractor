...
class N8NWorkflowMonitor:
    ...
    async def _make_api_request(self, endpoint: str, method: str = "GET", data: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    async def get_workflow_health(self, workflow_id: str = "") -> Dict[str, Any]:
        ...
        try:
            if workflow_id:
                ...
                workflow_data = await self._make_api_request(f"/api/v1/workflows/{workflow_id}")
                executions_data = await self._make_api_request(f"/api/v1/executions?workflowId={workflow_id}&limit=20")
                ...
                for workflow in workflows:
                    ...
                    try:
                        executions_data = await self._make_api_request(f"/api/v1/executions?workflowId={workflow_id}&limit=20")
                        workflow_health = self._analyze_workflow_health(workflow, executions_data)
                        ...
                        logger.warning(f"⚠️ Failed to analyze workflow {workflow_id}: {e}")
                        health_summary["problem_workflows"].append({
                            "id": workflow_id,
                            "name": workflow_name,
    ...
    def _analyze_workflow_health(self, workflow_data: Dict, executions_data: Dict) -> Dict[str, Any]:
        ...
        # Анализ executions
        executions = executions_data.get("data", []) if isinstance(executions_data.get("data"), list) else []
    ...
    async def analyze_workflow(self, workflow_id: str, analysis_type: str = "full") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            workflow_data = await self._make_api_request(f"/api/v1/workflows/{workflow_id}")
            executions_data = await self._make_api_request(f"/api/v1/executions?workflowId={workflow_id}&limit=50")
            ...
            if analysis_type in ["full", "performance"]:
                analysis["performance_analysis"] = self._analyze_performance(workflow_data, executions_data)
    ...
    def _analyze_performance(self, workflow_data: Dict, executions_data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "critical" if error_rate > 50 else "warning" if performance_issues else "good"
        }
    ...
    def _analyze_security(self, workflow_data: Dict) -> Dict[str, Any]:
            ...
            for key, value in parameters.items():
                if isinstance(value, str):
                    if any(secret in value.lower() for secret in ["password", "token", "key", "secret"]):
                        if not value.startswith("{{") and not value.startswith("$"):  # Не переменная окружения
        ...
        if secrets_found:
            security_issues.append(f"Found {len(secrets_found)} potential secrets in parameters")
        ...
        return {
            ...
            "status": "critical" if secrets_found else "warning" if security_issues else "good"
        }
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        if structure.get("complexity_score", 0) > 50:
            recommendations.append("Consider refactoring workflow - high complexity detected")
        
        if len(structure.get("triggers", [])) > 3:
            recommendations.append("Multiple triggers detected - consider consolidating")
        ...
        if performance.get("execution_times", {}).get("average", 0) > 30:
            recommendations.append("Slow execution time - optimize workflow performance")
