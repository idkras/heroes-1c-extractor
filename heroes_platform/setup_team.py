...
def install_node_dependencies():
    ...
    try:
        ...
        subprocess.run(['npm', 'install', 'figma-developer-mcp'], 
                      cwd=str(heroes_platform_path), check=True)
...
def setup_cursor_config():
    ...
    if not cursor_config_path.exists():
        print_status("Cursor MCP config not found. Please create it manually.", "WARNING")
        return False
