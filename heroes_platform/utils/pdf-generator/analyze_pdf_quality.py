...
def identify_issues(results):
    ...
    for filename, data in results.items():
        ...
        # Проблемы с размером
        if size_kb < 100:
            issues.append(f"❌ {filename}: Слишком маленький размер ({size_kb:.1f} KB) - возможно поврежден")
        elif size_kb > 10000:
            issues.append(f"⚠️ {filename}: Очень большой размер ({size_kb:.1f} KB) - возможно избыточное качество")
        
        # Проблемы с генераторами
        if "WeasyPrint" in generator:
            issues.append(f"❌ {filename}: WeasyPrint не работает на macOS из-за системных зависимостей")
    ...
    for expected in expected_generators:
        if expected not in found_generators:
            if "WeasyPrint" in expected:
                issues.append(f"❌ {expected}: Не работает на macOS (libgobject-2.0-0 отсутствует)")
            else:
