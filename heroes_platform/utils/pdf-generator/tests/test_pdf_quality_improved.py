...
def test_pdf_typography():
    ...
    assert "«" in text and "»" in text, (
        "Должны использоваться правильные русские кавычки"
    )
    ...
    assert " лет" not in text or "лет" in text, (
        "Должны использоваться неразрывные пробелы"
    )
...
def test_pdf_content_quality():
    ...
    assert "персональные данные" in text, (
        "Документ должен содержать информацию о персональных данных"
    )
...
def test_pdf_file_properties():
    ...
    assert file_size > 50000, (
        "PDF файл должен быть достаточно большим для качественной типографики"
    )
...
def test_improved_typography_features():
    ...
    if has_legal_refs:
        ...
        assert not has_bad_spacing, (
            "Юридические ссылки не должны содержать множественные пробелы"
        )
...
def test_comparison_with_basic_version():
    ...
    if Path(improved_path).exists() and Path(basic_path).exists():
        ...
        assert improved_size >= basic_size * 0.8, (
            "Улучшенная версия не должна быть значительно меньше базовой"
        )
