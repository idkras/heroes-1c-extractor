...
def extract_pdf_text(pdf_path):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("pdftotext недоступен, используем альтернативный метод")
            return None
...
def analyze_pdf_visual_issues(pdf_text):
    ...
    if "##" in pdf_text:
        issues.append(
            "КРИТИЧЕСКАЯ ПРОБЛЕМА: Символы ## найдены в тексте вместо заголовков"
        )
    else:
    ...
    if len(lines) < 20:
        issues.append(
            "КРИТИЧЕСКАЯ ПРОБЛЕМА: Слишком мало строк - документ может быть слит"
        )
    else:
