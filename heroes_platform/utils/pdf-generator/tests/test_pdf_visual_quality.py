...
def test_pdf_no_spacing_holes():
    ...
    assert not re.search(r"\s{3,}", text), (
        "Не должно быть больших промежутков между словами"
    )
...
def test_pdf_no_empty_pages():
    ...
    with open(pdf_path, "rb") as file:
        ...
        assert empty_pages < total_pages * 0.3, (
            f"Слишком много пустых страниц: {empty_pages} из {total_pages}"
        )
...
def test_pdf_no_green_headers():
    ...
    if Path(md_file).exists():
        ...
        assert "green" not in html_content.lower(), (
            "HTML не должен содержать зеленое оформление"
        )
        assert "#03A87C" not in html_content, (
            "HTML не должен содержать зеленый цвет #03A87C"
        )
        ...
        for page in reader.pages:
            ...
            if len(text) > 100:
                ...
                if content_lines:
                    ...
                    avg_line_length = sum(
                        len(line) for line in content_lines[:10]
                    ) / min(10, len(content_lines))
                    assert 30 <= avg_line_length <= 85, (
                        f"Средняя длина строки {avg_line_length} не в оптимальном диапазоне 30-85 символов"
                    )
                break
...
def test_pdf_no_weird_hyphens():
    ...
    assert not re.search(r"-\s+\w", text), (
        "Не должно быть дефисов с пробелами в середине слов"
    )
...
def test_comparison_with_broken_version():
    ...
    if Path(broken_path).exists():
        ...
        assert len(fixed_text) > 1000, (
            "Исправленный PDF должен содержать достаточно текста"
        )
