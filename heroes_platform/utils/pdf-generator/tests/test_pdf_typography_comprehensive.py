...
def test_pdf_line_width():
    ...
    pdf_generators = ["generate_pdf_final.py", "generate_pdf_typography_enhanced.py"]
...
def test_pdf_paragraph_structure():
    ...
    for i, paragraph in enumerate(paragraphs):
        if len(paragraph) > 500:  # Слишком длинный абзац
            ...
            if len(sentences) > 4:
                issues.append(
                    f"Абзац {i + 1}: {len(paragraph)} символов, {len(sentences)} предложений"
                )
...
def test_pdf_quotes_and_dashes():
    ...
    generators = ["generate_pdf_final.py", "generate_pdf_typography_enhanced.py"]
...
def test_pdf_non_breaking_spaces():
    ...
    for pattern, description in problems:
        ...
        if matches:
            issues.append(
                f"{description}: найдено {len(matches)} случаев без неразрывных пробелов"
            )
    ...
    for generator in generators:
        if Path(generator).exists():
            ...
            if "hyphens: auto" not in gen_content:
                issues.append(
                    f"{generator}: отсутствует автоматическая расстановка переносов"
                )
    ...
    try:
        ...
        result = subprocess.run(
            [
                "python",
                "-c",
                'from utils.typography_processor import TypographyProcessor; print("OK")',
            ],
            capture_output=True,
            text=True,
        )
