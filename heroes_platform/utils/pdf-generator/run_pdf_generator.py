...
try:
    from pdf_generator.generators.generate_pdf_final import (
        convert_md_to_pdf_final,  # type: ignore
    )

    from utils import create_quality_report, validate_markdown_content  # type: ignore
except ImportError:
...
def main():
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Только валидация входного файла"
    )
    parser.add_argument(
        "--quality-report", action="store_true", help="Создать отчет качества"
    )
    parser.add_argument(
        "--generator",
        choices=["final", "comprehensive", "playwright"],
        default="final",
        help="Выбор генератора PDF",
    )
    ...
    try:
        if args.generator == "final":
            ...
            from generators.generate_pdf_comprehensive_fix import (
                convert_md_to_pdf_comprehensive,
            )
