...
def comprehensive_typography_fix(text):
    ...
    text = re.sub(r'(\d+)\s+(лет|года|год|дней|день|часов|час|минут|минута)', rf'\1{nbsp}\2', text, flags=re.IGNORECASE)
    
    # Единицы измерения
    text = re.sub(r'(\d+)\s+(км|м|см|мм|кг|г|мг|л|мл)', rf'\1{nbsp}\2', text, flags=re.IGNORECASE)
    
    # Сокращения с номерами
    text = re.sub(r'(ст\.|п\.|ч\.|абз\.|рис\.|табл\.|гл\.)\s+(\d+)', rf'\1{nbsp}\2', text)
    text = re.sub(r'(№|No\.)\s+(\d+)', rf'\1{nbsp}\2', text)
    
    # Инициалы
    text = re.sub(r'([А-ЯЁ]\.)\s+([А-ЯЁ]\.)\s+([А-ЯЁ][а-яё]+)', rf'\1{nbsp}\2{nbsp}\3', text)
    text = re.sub(r'([А-ЯЁ]\.)\s+([А-ЯЁ][а-яё]+)', rf'\1{nbsp}\2', text)
    
    # Предлоги и союзы
    text = re.sub(r'\s+(а|и|в|к|с|о|у|за|на|до|от|по|для|при|без|под|над|про|через)\s+', rf' \1{nbsp}', text)
