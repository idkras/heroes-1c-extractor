...
def compare_pdf_tables():
    ...
    for pdf_file in pdf_files:
        ...
        print(
            f"   📊 Найдено паттернов таблиц: {table_analysis['table_patterns_found']}"
        )
        print(
            f"   📋 Строк структурированных данных: {table_analysis['structured_data_lines']}"
        )

        # Показываем примеры
        if table_analysis["sample_structured_data"]:
            print("   📝 Примеры структурированных данных:")
...
def analyze_code_blocks(text):
    ...
    return {"code_blocks_found": len(found_code), "code_types": list(set(found_code))}
...
def comprehensive_analysis():
    ...
    for pdf_file in pdf_files:
        ...
        table_score = min(
            10,
            table_analysis["table_patterns_found"] * 2
            + table_analysis["structured_data_lines"],
        )
...
def main():
    ...
    if comprehensive_results:
        best_file = max(
            comprehensive_results.items(), key=lambda x: x[1]["total_score"]
        )
