...
class CocoIndexBasicCreator:
    ...
    def analyze_script_categories(self, key_scripts: List[str]) -> Dict[str, List[str]]:
        ...
        for script in key_scripts:
            ...
            if any(keyword in script_name for keyword in ['workflow', 'orchestrator', 'pipeline']):
                categories['workflow'].append(script)
            elif any(keyword in script_name for keyword in ['monitor', 'log', 'health', 'check']):
                categories['monitoring'].append(script)
            elif any(keyword in script_name for keyword in ['util', 'helper', 'common']):
                categories['utility'].append(script)
            elif any(keyword in script_name for keyword in ['test', 'spec']):
                categories['test'].append(script)
            elif any(keyword in script_name for keyword in ['integrate', 'connect', 'sync']):
    ...
    def generate_basic_report(self, key_scripts: List[str], categories: Dict[str, List[str]]):
