...
try:
    from shared.credentials_manager import get_google_credentials, _get_google_access_token
except ImportError:
...
def create_campaign_template_sheet() -> str:
    ...
    try:
        ...
        spreadsheet_data = {
            "properties": {
                "title": "Шаблон разметки рекламных кампаний - Rick.ai"
            },
            "sheets": [
                {
                    "properties": {
        ...
        headers_data = [
            ["campaign_name", "campaign_type", "platform", "budget", "start_date", "end_date", "target_audience",
             "utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term",
             "adjust_tracker", "adjust_network", "adjust_campaign", "adjust_creative", "adjust_click_label",
             "appmetrica_source", "appmetrica_campaign", "appmetrica_content"]
        ]
        ...
        if response.status_code != 200:
            return f"❌ Error updating headers: {response.status_code} - {response.text}"
        
        # Добавляем примеры данных
        example_data = [
            ["vipavenue_ios_black_friday_2025", "yandex_direct", "ios", "50000", "2025-01-15", "2025-02-15", "women_25_45_fashion",
             "yandex_direct", "cpc", "vipavenue_ios_black_friday_2025", "banner_1200x628_red", "платье_вечернее",
             "abc123", "yandex_direct", "vipavenue_ios_black_friday_2025", "banner_1200x628_red", "women_25_45_fashion",
             "yandex_direct", "vipavenue_ios_black_friday_2025", "banner_1200x628_red"],
            ["vipavenue_android_cpa_lead_2025", "cpa_network", "android", "30000", "2025-01-15", "2025-02-15", "men_18_35_tech",
             "cpa_network", "banner", "vipavenue_android_cpa_lead_2025", "banner_728x90_blue", "смартфон_android",
             "def456", "cpa_network", "vipavenue_android_cpa_lead_2025", "banner_728x90_blue", "men_18_35_tech",
             "cpa_network", "vipavenue_android_cpa_lead_2025", "banner_728x90_blue"]
        ]
        ...
        if response.status_code != 200:
            return f"❌ Error updating examples: {response.status_code} - {response.text}"
        ...
        instructions_data = [
            ...
            [""],
            ["ОБЯЗАТЕЛЬНЫЕ ПОЛЯ:"],
            ["campaign_name", "Название кампании (без пробелов, используйте подчеркивания)"],
            ["campaign_type", "Тип кампании: yandex_direct, cpa_network, social"],
            ["platform", "Платформа: ios, android, desktop, multi"],
            ["utm_source", "Источник трафика: yandex_direct, cpa_network, facebook, etc."],
            ...
            ["adjust_campaign", "Название кампании в Adjust (должно совпадать с campaign_name)"],
            [""],
            ["РЕКОМЕНДУЕМЫЕ ПОЛЯ:"],
            ["utm_content", "Описание креатива: banner_1200x628_red, video_15s_fashion"],
