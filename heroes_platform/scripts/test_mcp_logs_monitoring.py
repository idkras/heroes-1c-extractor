...
class MCPLogsMonitor:
        ...
        try:
            ...
            if not self.cursor_logs_path.exists():
                ...
                recommendations.append(
                    "Установите Cursor IDE или проверьте путь к логам"
                )
                ...
                if not os.access(self.cursor_logs_path, os.R_OK):
                    ...
                    recommendations.append(
                        "Предоставьте права на чтение директории логов"
                    )
                    ...
                    if len(log_files) == 0:
                        ...
                        recommendations.append(
                            "Запустите Cursor IDE для создания лог-файлов"
                        )
            ...
            recommendations.append(
                "Проверьте системные права и доступность файловой системы"
            )
    ...
    def test_mcp_servers_logs(self) -> TestResult:
        ...
        try:
            ...
            if len(mcp_log_files) == 0:
                ...
                recommendations.append(
                    "Настройте MCP серверы в Cursor для создания логов"
                )
            else:
                # Анализируем каждый MCP лог-файл
                for log_file in mcp_log_files:
                    self._analyze_mcp_log_file(
                        log_file, errors, recommendations, evidence
                    )
            ...
            status = (
                ...
                if any(e.severity in ["medium", "low"] for e in errors)
                else "failed"
            )
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                # MCP сервер ошибки
                    ...
                    recommendations.append(
                        "Проверьте MCP сервер на соответствие протоколу"
                    )
                ...
                # Ошибки запуска
                    ...
                    recommendations.append(
                        "Используйте абсолютные пути в mcp.json конфигурации"
                    )
    ...
    def test_output_panel_simulation(self) -> TestResult:
        ...
        try:
            ...
            if mcp_channels_found == 0:
                ...
                recommendations.append(
                    "Настройте MCP серверы для появления в Output панели"
                )
    ...
    def test_credentials_wrapper_output(self) -> TestResult:
        ...
        try:
            ...
            if not credentials_wrapper_path.exists():
                ...
                with open(credentials_wrapper_path, encoding="utf-8") as f:
                    content = f.read()
                ...
                if "print(" in content and "file=sys.stderr" not in content:
                    ...
                    recommendations.append(
                        "Измените все print() на print(..., file=sys.stderr)"
                    )
                ...
                for emoji in emojis:
                    if emoji in content:
                        ...
                        recommendations.append(
                            "Убедитесь, что эмодзи выводятся в stderr"
                        )
    ...
    def run_all_tests(self) -> dict[str, Any]:
        ...
        for test_func in tests:
            ...
            print(
                f"   {status_emoji.get(result.status, '❓')} {result.test_name}: {result.status}"
            )
    ...
    def _generate_report(self) -> dict[str, Any]:
        ...
        critical_errors = sum(
            len([e for e in r.errors_found if e.severity == "critical"])
            for r in self.test_results
        )
        high_errors = sum(
            len([e for e in r.errors_found if e.severity == "high"])
            for r in self.test_results
        )
        ...
        report = {
            ...
            "errors": {
                ...
                "high": high_errors,
                "medium": sum(
                    len([e for e in r.errors_found if e.severity == "medium"])
                    for r in self.test_results
                ),
                "low": sum(
                    len([e for e in r.errors_found if e.severity == "low"])
                    for r in self.test_results
                ),
            },
