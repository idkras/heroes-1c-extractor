...
class DuplicateAnalyzer:
    ...
    def analyze_content_duplicates(self) -> Dict[str, List[str]]:
        ...
        # Find duplicates
        duplicates = {hash_val: files for hash_val, files in self.content_hashes.items() 
                     if len(files) > 1}
    ...
    def analyze_function_patterns(self) -> Dict[str, List[str]]:
        ...
        similar_functions = {pattern: funcs for pattern, funcs in function_patterns.items() 
                           if len(funcs) > 1}
    ...
    def analyze_class_patterns(self) -> Dict[str, List[str]]:
        ...
        similar_classes = {pattern: classes for pattern, classes in class_patterns.items() 
                          if len(classes) > 1}
    ...
    def _create_function_pattern(self, func_content: str) -> str:
        ...
        for line in lines:
            line = line.strip()
            if any(keyword in line for keyword in ['import', 'from', 'def', 'return', 'if', 'for', 'while', 'try', 'except']):
    ...
    def _create_class_pattern(self, class_content: str) -> str:
        ...
        for line in lines:
            line = line.strip()
            if any(keyword in line for keyword in ['class', 'def __init__', 'def ', 'import', 'from']):
                key_lines.append(line)
                
        return '|'.join(key_lines[:10])  # First 10 key lines
    
    def generate_report(self, duplicates: Dict, similar_functions: Dict, similar_classes: Dict):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if similar_functions:
                for pattern, funcs in list(similar_functions.items())[:10]:  # Top 10
                    f.write(f"### Similar Function Pattern\n")
            ...
            if similar_classes:
                for pattern, classes in list(similar_classes.items())[:10]:  # Top 10
                    f.write(f"### Similar Class Pattern\n")
                    f.write(f"**Pattern:** `{pattern[:100]}...`\n\n")
                    for cls in classes:
                        f.write(f"- `{cls}`\n")
                    f.write("\n")
