...
class FunctionalityMapper:
    ...
    def create_functionality_map(self) -> Dict[str, List[str]]:
        ...
        for category in self.functionality_map:
            self.functionality_map[category].sort(key=lambda x: x['score'], reverse=True)
            
        print(f"✅ Functionality map created with {len(self.functionality_map)} categories")
    ...
    def analyze_search_patterns(self) -> Dict[str, List[str]]:
        ...
        for category, files in self.functionality_map.items():
            ...
            if category == 'authentication':
                patterns.extend(['user login', 'password verification', 'session management'])
            elif category == 'database':
                patterns.extend(['database connection', 'sql queries', 'data persistence'])
            elif category == 'api':
                ...
                patterns.extend(['log monitoring', 'health checks', 'system monitoring'])
            elif category == 'workflow':
                patterns.extend(['workflow automation', 'process orchestration', 'pipeline management'])
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            for category, files in self.functionality_map.items():
                ...
                for file_info in files[:5]:  # Top 5 files
                    f.write(f"- `{file_info['file']}` (score: {file_info['score']:.2f})\n")
                f.write("\n")
