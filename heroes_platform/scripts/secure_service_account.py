...
def update_mcp_config():
    ...
    try:
        ...
        if "mcpServers" in config and "google-sheets" in config["mcpServers"]:
            config["mcpServers"]["google-sheets"]["env"]["GOOGLE_SERVICE_ACCOUNT_KEY"] = "keychain:google-service-account-json"
        
        # Обновить конфигурацию Google Drive
        if "mcpServers" in config and "google-drive" in config["mcpServers"]:
            config["mcpServers"]["google-drive"]["env"]["GOOGLE_SERVICE_ACCOUNT_KEY"] = "keychain:google-service-account-json"
