...
def get_service_account_key():
    ...
    try:
        ...
        json_string = subprocess.run([
            "xxd", "-r", "-p"
        ], input=hex_data, capture_output=True, text=True, check=True).stdout
            ...
            "sheets": [
                {
                    "name": "Campaigns",
                    "headers": [
                        "Parameter", "Status", "Android Tracking Link", "iOS Tracking Link", 
                        "Website Link", "Campaign", "Adgroup", "Creative", "Multi-App Link", 
                        "Deep Link", "Redirect Android", "Redirect iOS", "Yandex Direct Link", "Validation"
                    ],
                    "data": [
                        {
                ...
                },
                {
                    "name": "Examples",
                    "headers": [
                        "Campaign Type", "Campaign Name", "Campaign ID", "Adgroup", "Creative", 
                        "Deep Link", "UTM Source", "UTM Medium", "UTM Campaign", "UTM Content", "UTM Term", "Final Link"
                    ],
                    "data": [
                        {
...
def main():
    ...
    print('  =CONCATENATE("https://app.adjust.com/", B2, "?campaign=", F2, "&adgroup=", G2, "&creative=", H2, "&deep_link=", ENCODEURL(I2))')
    
    print("\nRedirect ссылка Android:")
    print('  =CONCATENATE("https://play.google.com/store/apps/details?id=com.vipavenue.android&referrer=", ENCODEURL(CONCATENATE("utm_source=adjust&utm_medium=app&utm_campaign=", F2)))')
    
    print("\nRedirect ссылка iOS:")
    print('  =CONCATENATE("https://apps.apple.com/de/app/vipavenue-%D0%B1%D1%80%D0%B5%D0%BD%D0%B4%D0%BE%D0%B2%D0%B0%D1%8F-%D0%BE%D0%B4%D0%B5%D0%B6%D0%B4%D0%B0/id1483431423?mt=8&ct=", ENCODEURL(F2))')
    
    print("\nФинальная ссылка для Yandex Direct:")
    print('  =CONCATENATE("https://vipavenue.ru?utm_source=yandex&utm_medium=cpc&utm_campaign=", F2, "&utm_content=", G2, "&utm_term=", H2)')
    
    print("\nВалидация:")
    print('  =IF(AND(LEN(F2)>0, LEN(G2)>0, LEN(H2)>0), "✅ Valid", "❌ Missing parameters")')
