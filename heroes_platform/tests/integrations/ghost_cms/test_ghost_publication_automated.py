...
class TestGhostPublication:
    ...
    def test_ghost_2025_connectivity(self):
        ...
        try:
            ...
            assert response.status_code == 200, f"Ghost 2025 недоступен: {response.status_code}"
    ...
    def test_ghost_2022_ru_connectivity(self):
        ...
        try:
            ...
            assert response.status_code == 200, f"Ghost 2022_RU недоступен: {response.status_code}"
    ...
    def test_url_accessibility(self, post_url: str):
        ...
        try:
            ...
            # Проверяем что URL доступен (200, 301, 302 - все OK)
            assert response.status_code in [200, 301, 302], f"URL недоступен: {response.status_code}"
            
            # Проверка что это не страница ошибки (игнорируем robots noindex)
            if "error" in response.text.lower() and "robots" not in response.text.lower():
            ...
            if img_tags:
                for img in img_tags:
                    ...
                    if src:
                        ...
                        assert img_response.status_code in [200, 301, 302], f"Изображение недоступно: {src}"
