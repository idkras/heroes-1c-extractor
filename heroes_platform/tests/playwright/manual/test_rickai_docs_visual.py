...
class TestRickAiDocsVisual:
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_header_white_no_shadow(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
        ...
        # Assertions
        assert "rgb(255, 255, 255)" in background_color, f"Header should be white, got: {background_color}"
        assert box_shadow == "none" or "0px" in box_shadow, f"Header should have no shadow, got: {box_shadow}"
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_left_navigation_visible(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_no_vertical_text_defect(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
        ...
        assert len(vertical_text_elements) == 0, f"Found vertical text defects: {vertical_text_elements}"
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_content_width_70_percent(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
        ...
        assert 65 <= content_percentage <= 75, f"Content should be ~70% width, got: {content_percentage:.1f}%"
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_details_sections_open(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
        ...
        assert len(open_details) == len(all_details), f"All details should be open. Found {len(open_details)}/{len(all_details)}"
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_font_size_15pt(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_no_overlapping_elements(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
        ...
        assert len(overlapping) <= 5, f"Too many overlapping elements: {len(overlapping)}"
    ...
    @pytest.mark.visual
    @pytest.mark.manual
    async def test_navigation_links_clickable(self, page: Page, screenshot_dir: Path, test_data: dict):
        """
        ...
        assert clickable_count >= len(nav_links) * 0.8, f"Most links should be clickable. {clickable_count}/{len(nav_links)}"
