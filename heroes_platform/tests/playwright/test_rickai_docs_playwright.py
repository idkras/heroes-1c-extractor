    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                screenshot_path = output_dir / f"01_header_white_no_shadow_{timestamp}.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
                
                test_result = {
                    "name": "header_white_no_shadow",
                    "status": "passed" if "rgb(255, 255, 255)" in background_color and (box_shadow == "none" or "0px" in box_shadow) else "failed",
            ...
            try:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = output_dir / f"02_left_navigation_visible_{timestamp}.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
                
                test_result = {
                    "name": "left_navigation_visible",
                    "status": "passed" if is_visible and len(nav_links) > 0 and position == "fixed" else "failed",
                    "screenshot": str(screenshot_path),
                    "details": {
                        "visible": is_visible,
            ...
            try:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = output_dir / f"03_no_vertical_text_defect_{timestamp}.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
                
                test_result = {
                    "name": "no_vertical_text_defect",
                    "status": "passed" if len(vertical_text_elements) == 0 else "failed",
                    "screenshot": str(screenshot_path),
                    "details": {
            ...
            try:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = output_dir / f"04_content_width_70_percent_{timestamp}.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
            ...
            try:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = output_dir / f"05_details_sections_open_{timestamp}.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
                
                test_result = {
                    "name": "details_sections_open",
                    "status": "passed" if len(all_details) > 0 and len(open_details) == len(all_details) else "failed",
                    "screenshot": str(screenshot_path),
                    "details": {
