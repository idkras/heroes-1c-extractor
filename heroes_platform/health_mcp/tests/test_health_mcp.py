...
class TestMCPServerLogValidator:
    ...
    async def test_check_import_errors(self):
        ...
        assert len(errors) == 3
        assert "ImportError: attempted relative import with no known parent package" in errors
...
class TestIntegration:
    ...
    async def test_health_check_all_servers_integration(self):
        ...
        with patch('health_mcp_server._test_server_health') as mock_health, \
...
async def test_mcp_server_logs_validation():
    ...
    assert len(failed_servers) == 0, f"Critical errors found in MCP servers: {failed_servers}"
...
async def test_mcp_server_health_check():
    ...
    failed_servers = [name for name, result in health_results.items() 
