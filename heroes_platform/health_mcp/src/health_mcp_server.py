...
def check_command_line_args():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "--help" or arg == "-h":
            ...
            print("Registered tools: health_check_all_servers, validate_server_logs, get_server_status, test_server_health")
            sys.exit(0)
            ...
            if log_dirs:
                ...
                for window_dir in latest_log_dir.iterdir():
                    if window_dir.is_dir() and window_dir.name.startswith('window'):
                        exthost_dir = window_dir / "exthost" / "anysphere.cursor-retrieval"
                        if exthost_dir.exists():
                            log_file = exthost_dir / f"MCP project-0-heroes-template-{self.server_name}.log"
                            if log_file.exists():
                                return str(log_file)
        return ""
    ...
    async def validate_server_logs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "warnings": warnings,
                "total_issues": len(critical_errors) + len(import_errors) + len(credential_errors) + len(connection_errors)
            }
