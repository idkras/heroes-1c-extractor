...
def check_command_line_args():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "--help" or arg == "-h":
            ...
            print("Registered tools: google_sheets_read_spreadsheet, google_sheets_write_data, google_sheets_list_spreadsheets, google_sheets_get_sheet_info, google_sheets_create_spreadsheet, google_sheets_delete_spreadsheet, google_sheets_share_spreadsheet, google_sheets_add_worksheet, google_sheets_delete_worksheet, google_sheets_rename_worksheet, google_sheets_copy_worksheet, google_sheets_get_formulas, google_sheets_batch_update, google_sheets_export_to_csv, google_sheets_import_from_csv")
            sys.exit(0)
...
@mcp.tool()
def google_sheets_read_spreadsheet(spreadsheet_id: str, range_name: str = "A1:Z100") -> str:
    """
    ...
    try:
        ...
        if not client:
            return json.dumps({
                "success": False,
                "error": "Не удалось создать клиент Google Sheets"
            }, ensure_ascii=False)
...
def google_sheets_delete_spreadsheet(spreadsheet_id: str) -> str:
    ...
    try:
        ...
        if not service_account_key:
            return json.dumps({
                "success": False,
                "error": "Не удалось получить Service Account ключ"
            }, ensure_ascii=False)
...
@mcp.tool()
def google_sheets_share_spreadsheet(spreadsheet_id: str, email: str, role: str = "reader") -> str:
    """
...
@mcp.tool()
def google_sheets_add_worksheet(spreadsheet_id: str, worksheet_name: str, rows: int = 1000, cols: int = 26) -> str:
    """
...
@mcp.tool()
def google_sheets_rename_worksheet(spreadsheet_id: str, old_name: str, new_name: str) -> str:
    """
...
@mcp.tool()
def google_sheets_copy_worksheet(spreadsheet_id: str, source_worksheet_name: str, new_worksheet_name: str) -> str:
    """
    ...
    try:
        ...
        # Копировать лист
        new_worksheet = spreadsheet.duplicate_sheet(source_worksheet.id, insert_sheet_index=None, new_sheet_name=new_worksheet_name)
...
@mcp.tool()
def google_sheets_export_to_csv(spreadsheet_id: str, worksheet_name: str = "Sheet1") -> str:
    """
...
@mcp.tool()
def google_sheets_import_from_csv(spreadsheet_id: str, csv_data: str, worksheet_name: str = "Sheet1", start_cell: str = "A1") -> str:
    """
