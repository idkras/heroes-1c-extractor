...
def get_service_credentials(service: str) -> dict:
    ...
    service_credentials = {
        ...
        ],  # Use Service Account JSON for Google Sheets MCP
        "google_oauth": [
    ...
    if service not in service_credentials:
        ...
        print(
            f"Available services: {', '.join(service_credentials.keys())}",
            file=sys.stderr,
        )
        return {}
...
def main() -> int:
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python credentials_wrapper.py SERVICE COMMAND [ARGS...]",
            file=sys.stderr,
        )
        print("Examples:", file=sys.stderr)
        print(
            "  python credentials_wrapper.py telegram python main.py", file=sys.stderr
        )
        print(
            "  python credentials_wrapper.py n8n node dist/mcp/index.js",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        process = subprocess.run(
            [command] + args, stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr
        )
