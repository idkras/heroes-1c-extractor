...
def get_figma_api_key():
    ...
    try:
        ...
        print(f"ERROR: Error getting Figma API key from credentials_manager: {e}", file=sys.stderr)
...
def main():
    ...
    figma_mcp_path = heroes_platform_path / "node_modules" / ".bin" / "figma-developer-mcp"
    if not figma_mcp_path.exists():
        print(f"ERROR: figma-developer-mcp not found at {figma_mcp_path}", file=sys.stderr)
        sys.exit(1)
