...
def check_imports():
    ...
    for module in critical_imports:
        success, stdout, stderr = run_command(f"python -c 'import {module}; print(f\"✅ {module} импортируется успешно\")'")
        if not success:
...
def check_mypy_errors():
    ...
    success, stdout, stderr = run_command("find . -name '*.py' -exec python -m mypy {} \\; 2>&1 | grep -c 'error:' || true")
...
def main():
    ...
    success, stdout, stderr = run_command("source .venv/bin/activate && python --version")
    if not success:
